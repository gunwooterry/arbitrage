...
class OrderItem(object):
    ...
    def __init__(self, order_id, info=None, initial_params=None, date=None):
        ...
        if info is not None:
            ...
            self.rate = decimal.Decimal(order['rate'])
            self.amount = decimal.Decimal(order['amount'])
            self.initial_rate = decimal.Decimal(order['initial_rate'])
            self.initial_amount = decimal.Decimal(order['initial_amount'])
        else:
            self.status = None
...
#         return balances
        info = self._post('getfunds', connection=connection, error_handler=error_handler)
        return info.get('available_funds', {}) # empty dict if no funds at all
    ...
    def placeOrder(self, pair, trade_type, rate, amount, connection=None, update_delay=None, error_handler=None):
        ...
        order = OrderItem(self._post('placeorder', params=params, connection=connection,
                                     error_handler=error_handler).get('order_id'), initial_params=params, date=now())
